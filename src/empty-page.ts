import { Plugin, TFile, TAbstractFile, FileView } from 'obsidian';
import EmptyPageTitlePlugin from './main';

export const EMPTY_PAGE_CLASS = 'is-empty-page';
export const EMPTY_PAGE_CLASS_ITALIC = 'is-empty-page-italic';

/**
 * Returns true when the given file is considered empty.
 *
 ...
 */
async function isFileEmpty(plugin: Plugin, file: TFile): Promise<boolean> {
	if (file.stat.size === 0) {
		return true;
	}
 ...
 *
 * Tab detection is done through the official API rather than by scraping the
 * DOM: we iterate over every markdown leaf and read `leaf.view.file` to obtain
 * the exact TFile it displays. The corresponding tab header element is then
 * located via the stable `data-leaf-id` attribute, which Obsidian sets on
 * every `.workspace-tab-header` and which matches `leaf.id` 1:1. This avoids
 * fragile `innerText` matching (which breaks if another plugin alters the
 * title) and the unreliable `data-path` attribute on the header element.
 */
async function updateFileStyling(plugin: Plugin, file: TFile): Promise<void> {
 ...
 */
export async function refreshAll(plugin: Plugin): Promise<void> {
...
export function registerEmptyPageStyling(plugin: Plugin): void {
	...
	// Re-evaluate deleted files (their DOM nodes are removed by Obsidian, but
